...
def calc_psnr(im1, im2):
    ...
    #ans = [compare_psnr(im1_y, im2_y)]   #deprecated
    ans = [psnr(im1_y, im2_y)]
    return ans
...
def calc_ssim(im1, im2):
    ...
    im1_y = cv2.cvtColor(im1, cv2.COLOR_BGR2YCR_CB)[:, :, 0]
    im2_y = cv2.cvtColor(im2, cv2.COLOR_BGR2YCR_CB)[:, :, 0]
    #ans = [compare_ssim(im1_y, im2_y)]  #deprecated
    ans = [ssim(im1_y, im2_y)]
    return ans
...
def to_ssim_skimage(pred_image, gt):
    ...
    ssim_list = [measure.ssim(pred_image_list_np[ind],  gt_list_np[ind], data_range=2, multichannel=True) for ind in range(len(pred_image_list))]
